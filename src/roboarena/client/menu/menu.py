...
from roboarena.shared.util import gradientRect
...
class Menu(ABC):
    ...
    def render_menu(self) -> None:
        self.ctx = RenderCtx(self.screen, Vector(0, 0), self.ctx.get_scale_cache())
        self.screen.blit(self.ctx.scale_texture(self.background_texture), (0, 0))
        for button in self.buttons.values():
            button.render_button(self.ctx)
        for text_field in self.text_fields.values():
            text_field.render_text(self.ctx)
        pygame.display.flip()
